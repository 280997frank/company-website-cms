...
import { EOrderType, ESortType } from "@/types/contactUs";
import {
  Breadcrumb,
  BreadcrumbItem,
  BreadcrumbLink,
...
const ContactUs: FC = () => {
  ...
  const { fetchPublish } = useLocationPublish({
    input: {
      page: currentPage,
      limit: 10,
      search: {
  ...
  const { fetchDelete } = useDeleteLocation({
    input: {
      page: currentPage,
      limit: 10,
      search: {
  ...
  return (
    <Layout title="Contact Us">
      <VStack align="flex-start" minHeight="100vh" padding={"10"}>
        ...
        <Table
          dropdownOptions={[{ value: "email", label: "CLIENT NAME" }]}
          caption="CLIENT LIST"
          tooltip="CLIENT TABLE"
          columnHeaders={[
            { name: "cityName", label: "CITY NAME", type: "link" },
            { name: "email", label: "EMAIL", type: "link" },
            { name: "isActive", label: "DISPLAY", type: "switch" },
          ]}
          data={responseListLocations}
          onSwitchChange={async (_, rowId) => {
            // console.log("rowId", rowId);
            const dataSwitch = responseListLocations.find(
              (data) => data.id == rowId
            );
          ...
          addNewButton={{ bgColor: "#FF558A", url: "/contact-us/new" }}
        />
        <ConfirmDelete
